...
def load_frame(frame: np.ndarray, columns: int, rows: int) -> list[str]:
...
def display_frames(final_display: list[list[str]], frame_rate: int) -> None:
...
def main():
    ...
    print(f'Progress : Converting frame {frame_num + 1} of {total_frames} to greyscale (took {end-start:.2f} seconds)')

    final_display: list[list[str]] = []
    start = time.perf_counter()
    ...
    print(f'Progress : Loading frame {frame_num} of {total_frames} (took {end-start:.2f} seconds)')
